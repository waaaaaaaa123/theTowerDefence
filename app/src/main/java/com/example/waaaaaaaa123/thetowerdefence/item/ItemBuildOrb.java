 ...
 */
public abstract class ItemBuildOrb extends Item {
    protected int orbId;
    @Override
    ...
    public void use() {
        switch (block.getId()){
            case Block.TOWER:
                Player.getTowerManager().getTower(block).addOrb(orbId);
                break;
            case Block.BASE:
                block.setId(Block.BUILD);
                if(Player.getGrid().getPath().contains(block)){
            ...
            case Block.BUILD:
                Player.getTowerManager().addTower(Tower.TOWER_ORB,block).addOrb(orbId);
                break;
        }
        super.use();
    }
